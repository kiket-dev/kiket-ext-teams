...
class TeamsNotificationExtension
  ...
  def acquire_access_token(context)
    ...
    raise ArgumentError, 'Missing Teams OAuth credentials' if [tenant_id, client_id, client_secret].any? { |value| value.to_s.strip.empty? }
    ...
    raise TeamsAPIError.new('Failed to obtain access token', status: response.code.to_i) unless response.is_a?(Net::HTTPSuccess)
  ...
  def build_message_payload(payload, _context)
  ...
  def parse_graph_response(response)
    ...
    message = (body.dig('error', 'message') || body['message'] if body.is_a?(Hash))
